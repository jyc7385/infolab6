#' Create a additive data
#'
#' additive function create a additive pheno and geno type data
#'
...
additive <- function(n, samp, mu, sigma, slope)
{
	mutation <-0.45  # default value

        change <-0

	value <- c(0,slope,slope*2)
  ...
  #libpath <- .libPaths()
  #geon_datapath <- "sample_genotype.txt"
  #geon_path <- paste(libpath, geon_datapath, sep = "")
  #pheno_datapath <- "sample_phenotype_add.txt"
  #pheno_path <- paste(libpath, pheno_path, sep = "")

  #write.table(geno, file=geon_path, row.names=F, col.names=F, quote=F,sep="\t")
  #write.table(pheno, file=pheno_path, row.names=F, col.names=F, quote=F,sep="\t")


	write.table(geno, file="sample_genotype.txt", row.names=F, col.names=F, quote=F,sep="\t")
  write.table(pheno, file="sample_phenotype_add.txt", row.names=F, col.names=F, quote=F,sep="\t")
